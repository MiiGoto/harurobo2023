...
CAN_message_t msg1;//can送信用buf
CAN_message_t msg2;//can送信用buf
...
//myservo.attach(23); 
  ...
  msg1.len = 8;
  msg1.id = 0x1FF;
  for ( int idx = 0; idx < msg1.len; ++idx ) {
    msg1.buf[idx] = 0;
  }
  msg2.len = 8;
  msg2.id = 0x100;
  for ( int idx = 0; idx < msg2.len; ++idx ) {
    msg2.buf[idx] = 0;
  }
...
void loop(void)
{
  ...
  float vx=map(testch[3], 364,1684,-250,250);
  float vy=map(testch[2], 364,1684,250,-250);
  float vt=(float)map(testch[0], 364,1684,200,-200);//ここが目標速度、この場合は前進方向に1m/s
  float L=0.825735308;
  ...
  /*float vg=map(testch[1], 364,1684,-8000,8000);
  if(vg*vg<49)vg=0;
  u[0] = pid4.pid_out(vg);
  */
   for (int i = 0; i < 4; i++) {
    msg1.buf[i * 2] = u[i] >> 8;
    msg1.buf[i * 2 + 1] = u[i] & 0xFF;
  }
  
if(((data[5]&0xC0)>>6)==1)myservo.write(80);
else myservo.write(150);

if(((data[5]&0x30)>>4)==1)msg2.buf[0]=1;
else msg2.buf[0]=0;
...
void timerInt() {
  while ( CANTransmitter.read(rxmsg) ) {
    ...
    if (rxmsg.id == 0x205) {
      pid4.now_value(rxmsg.buf[2] * 256 + rxmsg.buf[3]);
    }
     }
  if(flag)CANTransmitter.write(msg);
  if(flag)CANTransmitter.write(msg1);
  if(flag)CANTransmitter.write(msg2);
}
